...
{
    public class BasicPropertyManager : DynamicObject
    {
        ...
        private BasicPropertyManager(object o)
        {
            _object = o;
        ...
        public static dynamic Wrap(object o)
        {
            ...
            return new BasicPropertyManager(o) as dynamic;
        }
        ...
        public object this[string propertyPath]
        {
            get
            {
                ...
                if (!string.IsNullOrEmpty(propertyPath))
                {
                    ...
                    {
                        if (!TryGetByName(propertyName, out var reference))
                        {
                            result = Wrap(result)[propertyName];
                        }
                        else
                        {
                            result = reference;
            ...
            set
            {
                if(!string.IsNullOrEmpty(propertyPath))
                {
                    ...
                    var propertyParentName = string.Join(".", propertyNames.Take(propertyNames.Length - 1));
                    var propertyParent = this[propertyParentName];
                    _compiledProperties[propertyParent.GetType()][propertyName].Setter(propertyParent, value);
                }
            }
        }

        public static Type Type(object o, string propertyPath)
        {
            if (!string.IsNullOrEmpty(propertyPath))
            {
                var propertyValue = Wrap(o)[propertyPath];
        ...
        private bool TryGetByName(string memberName, out object result)
        {
            result = null;
        ...
        {
            return TrySetByName(binder.Name, value);
        }

        public bool TrySetByName(string memberName, object value)
        {
            Dictionary<string, GetterAndSetter> dict;
            GetterAndSetter getterAndSetter;
            if (false == _compiledProperties.TryGetValue(_objectType, out dict)
